...
<div class="container">
		<div class="content">
			...
			if(isset($_GET['aksi']) == 'delete'){
				$id = $_GET['id_topik'];
				$cek = mysqli_query($koneksi, "SELECT * FROM topik_kursus WHERE id_topik='$id'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-info">Data tidak ditemukan.</div>';
				}else{
					$delete = mysqli_query($koneksi, "DELETE FROM topik_kursus WHERE id_topik='$id'");
					if($delete){
			...
			<a href="materikursusadd.php" class="btn btn-primary">Tambah</a>
			<br>
			...
			<table class="table table-striped table-hover">
				...
				<?php
				if($urut){
					$sql = mysqli_query($koneksi, "SELECT * FROM topik_kursus WHERE status='$urut' ORDER BY id_topik ASC");
				}else{
					$sql = mysqli_query($koneksi, "SELECT * FROM topik_kursus ORDER BY id_topik ASC");
				}
