...
<div class="container">
		<div class="content">
			...
			<?php
			$id = $_GET['id_topik'];
			$sql = mysqli_query($koneksi, "SELECT * FROM topik_kursus WHERE id_topik='$id'");
			...
			if(isset($_POST['save'])){
				...
				$update = mysqli_query($koneksi, "UPDATE `topik_kursus` SET `topik`='$topik',`tgl_mulai`='$tglmulai',`tgl-akhir`='$tglakhir',`deskripsi`='$deskripsi',`file`='$file',`jenis_level`='$level',`status`='$status',`kuota`='$kuota' WHERE id_topik='$id'") or die(mysqli_error());
				if($update){
					
					header("Location: edit.php?id_topik=".$nik."&pesan=sukses");
				}else{
