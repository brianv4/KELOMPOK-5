...
//bentuk sql query untuk update
    $update = "update galeri set deskripsi='".$_POST['deskripsi']."' ";
    if($_FILES['file']['size'] > 0 && $_FILES['file']['error'] == 0){ //update gambar hanya jika user memilih file baru
        $move = move_uploaded_file($_FILES['file']['tmp_name'], "../../uploadpengurus/".$_FILES['file']['name']);
        if($move){
...
<body>
    <form name="form1" action="" method="post" enctype="multipart/form-data">
        <?php
        $sql = "select * from galeri where id='".intval($_GET['id'])."'";
        $data = mysql_fetch_array(mysql_query($sql));
        ?>
        <!-- menampilkan gambar sebelumnya -->
        <img src="../../uploadpengurus/<?php echo $data['nama_file'];?>" alt="" width="200"/><br/>
